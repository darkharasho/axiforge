...
function renderTableView(container) {
  ...
  const folderRows = folders
    .map(
      (f) => `
        <tr class="lib-table__row lib-table__row--folder" data-folder-id="${escapeHtml(f.id)}">
          <td class="lib-table__td lib-table__td--pin"></td>
          <td class="lib-table__td lib-table__td--icon">${folderIcon}</td>
          <td class="lib-table__td lib-table__td--name" colspan="6">${escapeHtml(f.name)}</td>
        </tr>
      `
    )
    .join("");
