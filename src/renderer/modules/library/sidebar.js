...
function renderSmartFolders(profExpanded, modeExpanded) {
  ...
  // The spec pins this order: "All Builds (by folder)" sits directly under
  // "Main Repository" because two rows with the same count and different
  // contents read as a bug unless the pair is adjacent enough to compare.
  const allBuildsRow = `
      <button type="button"
  ...
  `;
  const builtinRows = builtins.map((f) => smartRowHtml(f));
  const mainAt = builtins.findIndex((f) => f.id === "__sf-main");
  builtinRows.splice(mainAt >= 0 ? mainAt + 1 : 0, 0, allBuildsRow);
