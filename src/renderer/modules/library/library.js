...
/**
 * `state.currentFolder.smartFolder` is a snapshot taken when the row was
 * clicked, so editing, renaming or deleting the folder you are looking at
 * would otherwise keep filtering through the pre-edit record -- or point at a
 * folder that no longer exists at all. Re-resolving here, before anything
 * renders, covers all three: the sidebar count, the list contents and the
 * breadcrumb all come back from the same, current record.
 */
function revalidateCurrentSmartFolder() {
  const current = state.currentFolder;
  if (current?.type !== "smart-rule" || typeof current.id !== "string") return;
  // getSmartFolder resolves built-ins (including hidden ones, so a stale
  // navigation still works) and user records; the sidebar resolver rebuilds
  // the generated By Profession / By Game Mode rows, which are never persisted.
  const resolved = getSmartFolder(current.id) || resolveSidebarSmartFolder(current.id);
  if (!resolved) {
    // Spec: navigating to a smart folder that no longer exists falls back to
    // All Builds (by folder).
    state.currentFolder = { type: "all" };
    return;
  }
  if (resolved !== current.smartFolder) state.currentFolder = { ...current, smartFolder: resolved };
}
...
async function handleRenameSmartFolder(sf) {
  ...
  // Without the override, clearing the input and blurring would rename the
  // smart folder to "New Folder" -- insertInlineInput's default fallback.
  const newName = await insertInlineInput(navItem, sf.name || "", { fallbackName: sf.name || "" });
...
// Exported (unlike its sibling handlers) only so the hide-refusal unit test can
// drive it directly -- there is no DOM path to a context-menu item from jsdom.
