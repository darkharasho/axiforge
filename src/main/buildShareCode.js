...
function hasProfessionData(build) {
  ...
  if (prof === "Warrior") {
    // activeWeaponSet: 1 is default (set 1), 2 means set 2
...
function encodeShareCode(build) {
  ...
  if (hasProfData) {
    ...
    if (prof === "Revenant") {
      ...
      w.write(0, 1); // active legend slot (default 0)
...
function parseShareCodeWrapper(code) {
  ...
  const inner = code.slice(SHARE_CODE_PREFIX.length, -1); // strip <AxiForge: and >
...
function decodeShareCode(code) {
  ...
  // (per-slot stats reading omitted for v1)
