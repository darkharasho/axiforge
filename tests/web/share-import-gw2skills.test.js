...
describe("share.importGw2Skills", () => {
  it("makes one request to the Worker with url + gameMode, returns the Worker's build", async () => {
  ...
  it("surfaces the Worker's error + detail", async () => {
    const share = createShareApi({
      fetch: fakeFetch({ ok: false, status: 502, body: { error: "gw2skills.net could not be reached.", detail: "boom" } }),
    });
    await expect(
      share.importGw2Skills("https://en.gw2skills.net/editor/?abc")
    ).rejects.toThrow(/could not be reached.*boom/);
  });
});
