...
const ROUTES = [
  ...
  ["GET",    /^\/teams\/(?<teamId>[^/]+)\/grants$/,              teams.listGrants,       true],
  ["PUT",    /^\/teams\/(?<teamId>[^/]+)\/grants\/(?<folderId>[^/]+)\/(?<userId>[^/]+)$/, teams.setGrant,   true],
  ["DELETE", /^\/teams\/(?<teamId>[^/]+)\/grants\/(?<folderId>[^/]+)\/(?<userId>[^/]+)$/, teams.clearGrant, true],
